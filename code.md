---
title: code
---

## Ruby Projects

- Maintainer of [Boxr - Box Ruby Client](https://github.com/cburnette/boxr)
- [GroceryGuru (Server) - Grocery Spending Tracking](https://github.com/xhocquet/groceryguru-server)
- [DIYRSS - Turn any website into a trackable content feed](https://github.com/xhocquet/diyrss)
- [Flutter - Anime tracking data visualizer](https://github.com/xhocquet/Flutter)

## Game Projects

(recent to oldest)

- [SandGit - s&box Editor Plugin](https://github.com/xhocquet/SandGit)
  - s&box Link: [SandGit](https://sbox.game/meesles/sandgit)
- [Chiptasia - Rails/Godot](https://chiptasia.com)
- [Glue Prince - Gamemaker Studio 2](https://github.com/xhocquet/glue-prince)
  - Blog post: [Glue Prince](/reflections/glue-prince-wrap-up/)
- [GD-Sync Implementation](/reflections/2025-12-15-reflections-gd-sync-implementation)
- [Godot Waterways Library Upgrade](https://github.com/Arnklit/Waterways/pull/29#issuecomment-3663252192)
- [Godot 2D Platformer Enemy AI Demo](/reflections/2026-02-01-pathfinding) - [GitHub](https://github.com/xhocquet/2d-platformer-pathfinding-demo)

## Go Projects

- [PDFtools](https://github.com/xhocquet/pdf-tools)

## Javascript Projects

- [Waves - Electron music player with hot-loading](https://github.com/xhocquet/Waves)
- [GroceryGuru (Client) - React Native client for Rails API](https://github.com/xhocquet/groceryguru-native)
- [GraphQL Experiment](https://github.com/xhocquet/GraphQL-NextJS-Appolo-)
- [Meteor Experiment - Task list](https://github.com/xhocquet/meteor-tasks)
- [Electron Agile Tool](https://github.com/xhocquet/EpicBoards)

## Rust Projects

- [Personal Dashboard - Playing with Rust's Rocket framework](https://github.com/xhocquet/rust-personal-dashboard/)
- [Rustlings](https://github.com/xhocquet/rustlings)
